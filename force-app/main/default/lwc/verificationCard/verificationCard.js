...
export default class VerificationCard extends LightningElement {
    ...
    handleEditClick(event) {
        ...
        this.editingFields = row.displayFields.map(f => ({
            ...
            editValue: f.isBoolean ? String(f._boolValue === true) : (f.displayValue === '—' ? '' : f.displayValue),
    ...
    handleEditChange(event) {
        ...
        const value = isCheckbox ? String(event.target.checked) : event.target.value;

        let fields = this.editingFields.map(f =>
            f.apiName === name
                ? { ...f, editValue: value, editChecked: isCheckbox && event.target.checked }
                : f
        );
        ...
        if (name === 'Verification_Status__c') {
            ...
            fields = fields.map(f =>
                f.apiName === 'Is_Verified__c'
                    ? { ...f, editChecked: nowVerified, editValue: String(nowVerified) }
                    : f
            );
        } else if (name === 'Is_Verified__c') {
            const isChecked = event.target.checked;
            fields = fields.map(f =>
                f.apiName === 'Verification_Status__c'
                    ? { ...f, editValue: isChecked ? 'Verified' : 'Proposed' }
                    : f
            );
        }
    ...
    async handleEditSave() {
        ...
        for (const f of this.editingFields) {
            ...
            if (f.isBoolean) {
                ...
                const origBool = originalField ? originalField._boolValue === true : false;
                if (newBool !== origBool) {
